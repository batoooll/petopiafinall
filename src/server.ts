...
let _aiProc: ChildProcess | null = null;
...
async function startAIService() {
  ...
  const aiDir  = path.resolve(process.cwd(), "ai-service");
  const script = path.join(aiDir, "app.py");
  const cmd    = process.env.PYTHON_CMD ?? (process.platform === "win32" ? "python" : "python3");

  _aiProc = spawn(cmd, [script], {
    cwd: aiDir,
  ...
  _aiProc.on("error", err =>
    console.error(`[AI] could not start — is Python in PATH? (${err.message})`)
  );
