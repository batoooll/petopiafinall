...
class VetService {
    ...
    static async completeAppointment(userId, appointmentId, notes) {
        const result = await vets_repository_1.VetRepository.markAppointmentComplete(appointmentId, userId, notes);
        if (!result) {
            throw new AppError_1.AppError('Appointment not found or does not belong to you', AppError_1.HttpCode.NOT_FOUND);
        }
        return result;
    }
}
