...
class SittingController {
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    static async uploadIdCardPhoto(req, res, next) {
        try {
            if (!req.user?.userId) {
                throw new AppError_1.AppError("Unauthorized", AppError_1.HttpCode.UNAUTHORIZED);
            }
            ...
            const parsed = sitting_dto_1.UploadIdCardPhotoSchema.safeParse(req.body);
            if (!parsed.success) {
                return res.status(AppError_1.HttpCode.BAD_REQUEST).json({
                    success: false,
                    message: "Validation failed",
            ...
            const result = await sitting_service_1.SittingService.uploadIdCardPhoto(req.user.userId, req.file, parsed.data);
            return res.status(AppError_1.HttpCode.CREATED).json({
                success: true,
                message: result.message,
                data: result,
    ...
    static async uploadLocationPhoto(req, res, next) {
        try {
            if (!req.user?.userId) {
                throw new AppError_1.AppError("Unauthorized", AppError_1.HttpCode.UNAUTHORIZED);
            }
            ...
            const parsed = sitting_dto_1.UploadLocationPhotoSchema.safeParse(req.body);
            if (!parsed.success) {
                return res.status(AppError_1.HttpCode.BAD_REQUEST).json({
                    success: false,
                    message: "Validation failed",
            ...
            const result = await sitting_service_1.SittingService.uploadLocationPhoto(req.user.userId, req.file, parsed.data);
            return res.status(AppError_1.HttpCode.CREATED).json({
                success: true,
                message: result.message,
                data: result,
    ...
    static async getVerificationPhotos(req, res, next) {
        try {
            if (!req.user?.userId) {
                throw new AppError_1.AppError("Unauthorized", AppError_1.HttpCode.UNAUTHORIZED);
            }
            const photos = await sitting_service_1.SittingService.getVerificationPhotos(req.user.userId);
            return res.json({
                success: true,
